...
.title { background-color:#cce792;padding-top:5px;padding-bottom:5px;padding-right:5px;border-bottom:1px solid #fff; }
...
<?php
        ...
        $max = 50; 
        //Leden tellen
        $aantal_leden = DB::exQuery("SELECT id FROM transferlist_log")->num_rows; 
...
$messagesquery = DB::exQuery("SELECT * FROM transferlist_log
  ...
  //Als er meer dan 1 bericht is
	...
	{  
  	...
  	echo '<center><table width=700" border="0" cellspacing="0" cellpadding="0" style="text-align:center;">
			<tr>
				<td width="120" class="top"><b>Comprador</b></td>
				<td width="80" class="top"><b>Vendedor</b></td>
				<td width="150" class="top"><b>Quando</b></td>
				<td width="200" class="top"><b>Pokémon</b></td>
				<td width="200" class="top"><b>Valor</b></td>
			</tr>
			<tr>
				<td colspan="4"></td>
			</tr>';
	...
	$info2 = DB::exQuery("SELECT * FROM pokemon_wild WHERE wild_id = '".$row['wild_id']."'")->fetch_assoc();
			...
			echo'<tr>
					<td class="user"><a href="./profile&player='.$row['buyer'].'">'.$row['buyer'].'</a><span style="float:left;"></span></td>
					<td class="user"><a href="./profile&player='.$row['seller'].'">'.$row['seller'].'</a></td>
					<td class="title"><b>'.date('d/m/Y H:m:s', strtotime($row['date'])).'</b></td>
					<td class="user"><b>#'.$info2['real_id'].' - '.$info2['naam'].' <br> Level '.$row['level'].'</b></td>
					<td class="title"><b>Silver: '.number_format($row['silver']).' <br> Gold: '.$row['gold'].'</b></td>
				</tr>
				<tr>
					<td colspan="4"></td>
				</tr>';
