...
<?php
        ...
        $max = 50; 
        //Leden tellen
        $aantal_leden = DB::exQuery("SELECT id FROM bank_logs")->num_rows;
...
$messagesquery = DB::exQuery("SELECT * FROM bank_logs
  ...
  //Als er meer dan 1 bericht is
	...
	{  
	  //Scherm weergave
  	echo '<table width=670" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td width="130" class="top"><b>De</b></td>
				<td width="110" class="top"><b>Para</b></td>
				<td width="150" class="top"><b>Quando</b></td>
				<td width="280" class="top"><b>Quanto</b></td>
			</tr>
			<tr>
				<td colspan="4"></td>
			</tr>';
        ...
        {
        $row2 = $messagesquery2->fetch_assoc();
			 $j++;
			...
			echo'<tr>
					<td class="user"><a href="./profile&player='.$row['sender'].'">'.$row['sender'].'</a><span style="float:left;"></span></td>
					<td class="user"><a href="./profile&player='.$row['reciever'].'">'.$row['reciever'].'</a></td>
					<td class="title"><b>'.implode("/",array_reverse(explode("-",$row['date']))).'</b></td>
					<td width="320" class="massege"><table width="320" border="0">
							<tr>
								<td width="320"><img src="'.$static_url.'/images/icons/'.$row['what'].'.png"> '.$row['amount'].'</td>
							</tr>
						</table></td>
				</tr>
				<tr>
